...
impl Workspace {
    ...
    pub fn load(root: PathBuf) -> Result<Self> {
        ...
        let config: WorkspaceConfig = toml::from_str(&config_content)
            .map_err(|e| TtError::IoError(std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string())))?;
    ...
    fn discover_projects(projects_dir: &Path) -> StorageResult<HashMap<String, Project>> {
        ...
        for entry in entries {
            ...
            let config: ProjectConfig = toml::from_str(&config_content)
                .map_err(|e| StorageError::IoError(std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string())))?;
